...
const AddProduct = () => {
  ...
  useEffect(() => {
    if (formData.category) {
      ...
      // Show tech stack for Electronics
      setShowTechStack(selectedCategory?.name?.toLowerCase() === "electronics");
      // Fetch brands for this category
      fetchBrands().then((allBrands) => {
        // If category has brands, filter them, else show all
        if (selectedCategory?.brands?.length > 0) {
          setBrands(
            allBrands.filter((b) => selectedCategory.brands.includes(b._id))
          );
        } else {
          setBrands(allBrands);
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        ...
        } else if (key === "brand" && formData.brand) {
          productData.append("brand", formData.brand);
        } else if (key === "techStack" && formData.techStack) {
          productData.append("techStack", formData.techStack);
        } else {
    ...
    <div className="admin-products">
      ...
      <div className="products-container">
        ...
        >
          ...
          {showTechStack && (
            <div className="form-group tech-stack-group">
              <label htmlFor="techStack">Tech Stack / Technical Details</label>
              <textarea
                id="techStack"
                name="techStack"
                ...
                rows="3"
                placeholder="Enter technical details (e.g. RAM, Storage, Processor, etc.)"
              />
            </div>
          )}
